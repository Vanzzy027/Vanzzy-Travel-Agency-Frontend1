...
const Hero: React.FC = () => {
  ...
  return (
    <div className="relative w-full h-[70vh] min-h-[600px] overflow-hidden bg-[#001524]">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="text-center text-[#E9E6DD] max-w-4xl px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <button 
              onClick={() => handleProtectedNavigation(slides[currentSlide].action)}
              className="bg-[#F57251] text-[#E9E6DD] px-8 py-4 rounded-full font-semibold hover:bg-[#e56546] transform hover:scale-105 transition-all duration-300 shadow-lg text-lg"
            >
            ...
            <button 
              ...
              className="border-2 border-[#E9E6DD] text-[#E9E6DD] px-8 py-4 rounded-full font-semibold hover:bg-[#E9E6DD] hover:text-[#001524] transform hover:scale-105 transition-all duration-300 text-lg"
            >
              Learn More
            </button>
          </div>
      ...
      {/* Arrows */}
      <button onClick={prevSlide} className="absolute left-4 top-1/2 ...">…</button>
      <button onClick={nextSlide} className="absolute right-4 top-1/2 ...">…</button>

      {/* Indicators */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex space-x-3">
        {slides.map((_, index) => (
          <button key={index} onClick={() => setCurrentSlide(index)} className="w-3 h-3 rounded-full transition-all duration-300" />
        ))}
      </div>
    </div>
  );
