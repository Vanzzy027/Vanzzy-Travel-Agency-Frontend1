 ...
 */
const buildSanitizedClone = async (
 ...
 */
const generatePDFBlob = async (receiptElement: HTMLElement): Promise<Blob> => {
...
const ReceiptModal: React.FC<ReceiptModalProps> = ({
  ...
  const shareReceipt = async () => {
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        alert(
          "Receipt details copied to clipboard! You can also download the PDF.",
        );
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-[#001524]/80 backdrop-blur-md z-50 flex items-center justify-center p-4 sm:p-6">
      <div className="bg-[#E9E6DD] rounded-2xl w-full max-w-4xl max-h-[95vh] flex flex-col shadow-2xl overflow-hidden border border-[#EDE6E1]">
        {/* Header (hidden on print) */}
        <div className="flex items-center justify-between p-4 bg-white/50 backdrop-blur-sm border-b border-[#EDE6E1] print:hidden">
        ...
        {/* Receipt (exact visual) */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-8">
          <div
            ref={receiptRef}
            className="max-w-3xl mx-auto bg-white rounded-xl overflow-hidden shadow-xl border border-[#EDE6E1]"
          >
            {/* ... rest of receipt JSX unchanged ... */}
            {/* Top Section */}
            <div className="bg-[#001524] px-6 sm:px-8 pt-8 pb-8 flex flex-col sm:flex-row justify-between items-center sm:items-start gap-4 border-b-4 border-[#027480]">
              <div className="flex items-start gap-4 w-full sm:w-auto">
                <div className="w-20 h-20 rounded-full bg-white p-1.5 shadow-lg border-2 border-[#E9E6DD] shrink-0 flex items-center justify-center overflow-hidden mt-1">
                  <img
                    src="/logo.png"
                    alt="VansKE"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[#E9E6DD] text-xs space-y-2 font-medium">
                    <p className="flex items-start gap-2">
                      <Phone className="w-3.5 h-3.5 text-[#F57251] shrink-0 mt-0.5" />
                      <span>+254 112 178 578</span>
                    </p>
                    <p className="flex items-start gap-2">
                      <Mail className="w-3.5 h-3.5 text-[#F57251] shrink-0 mt-0.5" />
                      <span className="break-all">receipt@vansrental.com</span>
                    </p>
                    <p className="flex items-start gap-2">
                      <Globe className="w-3.5 h-3.5 text-[#F57251] shrink-0 mt-0.5" />
              ...
              <div className="text-center sm:text-right mt-4 sm:mt-0 shrink-0">
                ...
                <p className="text-2xl font-mono font-bold text-white bg-white/10 px-3 py-1.5 rounded-md backdrop-blur-sm border border-white/20 shadow-inner">
                  {receiptNumber}
                </p>
              </div>
            </div>
            ...
            <div className="p-6 sm:p-8">
              {/* Paid Stamp */}
              <div className="flex flex-col sm:flex-row items-center justify-between bg-linear-to-r from-[#F6F5F1] to-white rounded-xl p-6 border border-[#EDE6E1] shadow-sm mb-8 relative overflow-hidden">
                ...
                <div className="mt-4 sm:mt-0 relative z-10 transform sm:rotate-12">
                  <div className="border-4 border-[#F57251] text-[#F57251] rounded-lg px-6 py-2 text-2xl font-black tracking-widest uppercase shadow-lg bg-white/80 backdrop-blur-sm">
                    PAID
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="text-[#445048] text-sm mt-2 space-y-1">
                    ...
                    {booking.license_plate && (
                      <p>
                        Plate:{" "}
                        <span className="font-mono bg-[#E9E6DD] px-2 py-0.5 rounded text-[#001524] font-bold shadow-sm">
                          {booking.license_plate}
                        </span>
                      </p>
                    )}
              ...
              <div className="w-full border-t-2 border-dashed border-[#C4AD9D] my-8 relative">
                <div className="absolute -left-10 top-1/2 -translate-y-1/2 w-6 h-6 bg-[#E9E6DD] rounded-full border-r border-[#EDE6E1] print:hidden shadow-inner"></div>
                <div className="absolute -right-10 top-1/2 -translate-y-1/2 w-6 h-6 bg-[#E9E6DD] rounded-full border-l border-[#EDE6E1] print:hidden shadow-inner"></div>
              </div>

              {/* Rental period & QR */}
              <div className="flex flex-col md:flex-row justify-between gap-8 items-center">
                <div className="flex-1 w-full bg-linear-to-br from-[#012B38] to-[#014452] rounded-xl p-6 text-white shadow-lg relative overflow-hidden border border-[#014452]">
                  {/* Soft overlay for readability */}
                  <div className="absolute inset-0 bg-white/5 rounded-xl"></div>
                  <h3 className="flex items-center gap-2 text-[#0a0a09] text-sm font-bold uppercase mb-4 relative z-10">
                    <Calendar className="w-4 h-4 text-[#F57251]" /> Rental
                    Period
                  </h3>
                  <div className="flex items-center justify-between relative z-10">
                    <div>
                      <p className="text-xs text-[#2c221b] mb-1">Pick-up</p>
                      <p className="font-bold text-black text-lg">
                        {formatDate(booking.booking_date)}
                      </p>
                    </div>
                    <div className="h-0.5 w-12 bg-[#027480]/50"></div>
                    <div className="text-right">
                      <p className="text-xs text-[#2c221b] mb-1">Return</p>
                      <p className="font-bold text-black text-lg">
                ...
                <div className="flex flex-col items-center justify-center shrink-0">
                  <div className="p-2 border-2 border-[#E9E6DD] rounded-xl bg-white shadow-sm">
                    <QRCodeSVG
                      value={qrCodeData}
                      size={100}
                      level="M"
                  ...
                  <p className="text-[10px] text-[#F57251] mt-2 uppercase font-bold tracking-widest">
                    Official QR Code
                  </p>
                </div>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-[#E9E6DD] flex flex-col md:flex-row justify-between gap-6">
                ...
                <div className="flex-1 flex flex-col justify-end text-left md:text-right">
                  ...
                  <p className="text-xs font-mono text-[#837a73]">
        ...
        {/* Action buttons */}
        <div className="p-4 sm:p-6 bg-white/50 backdrop-blur-md border-t border-[#EDE6E1] flex flex-wrap gap-3 justify-end items-center print:hidden">
          <button
