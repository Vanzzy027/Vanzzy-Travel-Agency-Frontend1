...
import { Home, Car, CalendarDays, CreditCard, HelpCircle, LogOut, User, ChevronLeft, ChevronRight, Menu, } from "lucide-react";
const UserSidebar = ({ isCollapsed, onToggleCollapse, isMobile, }) => {
    const { user } = useSelector((state) => state.auth);
    const userFromStorage = JSON.parse(localStorage.getItem("user") || "{}");
    const displayUser = user || userFromStorage;
    const userInitial = displayUser?.first_name
    ...
    const menuItems = [
        ...
        { icon: Car, label: "Browse Vehicles", path: "/UserDashboard/vehicles" },
        {
            icon: CalendarDays,
            label: "My Bookings",
            path: "/UserDashboard/my-bookings",
        },
              ...
              `;
                            if (isButton) {
                                return (_jsx("button", { onClick: () => {
                                        item.action?.();
                                        handleLinkClick();
                                    }, className: commonClasses, children: content }, index));
                            }
